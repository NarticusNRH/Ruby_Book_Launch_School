...
Now that we're removing an element from names upon each iteration, we can stop the loop once the array is empty.
Array#empty? is a really handy method for situations like this. #empty? simply checks to see if there are any elements in an array and returns a boolean. 
